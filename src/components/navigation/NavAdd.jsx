...
import { createNavCard } from "/services/navService";
...
export const NavAdd = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
        ...
        if (img) {
          const imgRef = ref(storage, `card-images/${Date.now()}-${img.name}`);
          await uploadBytes(imgRef, img);
          imageUrl = await getDownloadURL(imgRef);
        }
        
        const navData = { title, url, imageUrl };
        console.log("Submitting:", navData); 
        await createNavCard(navData);
      ...
      <Dialog open={open} onOpenChange={setOpen} className="p-10 h-full">
        ...
        <DialogContent className="bg-black">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <InputImg onChange={(file) => setImg(file)} />
